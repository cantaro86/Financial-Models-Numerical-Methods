...
class Merton_process:
    ...
    def __init__(self, r=0.1, sig=0.2, lam=0.8, muJ=0, sigJ=0.5):
        ...
        self.skew = (
            self.lam
            * (3 * self.sigJ**2 * self.muJ + self.muJ**3)
            / self.var ** (1.5)
        )
        self.kurt = (
            self.lam
            * (3 * self.sigJ**3 + 6 * self.sigJ**2 * self.muJ**2 + self.muJ**4)
            / self.var**2
        )
    ...
    def exp_RV(self, S0, T, N):
        ...
        P = ss.poisson.rvs(
            self.lam * T, size=N
        )  # Poisson random vector (number of jumps)
        Jumps = np.asarray(
            [ss.norm.rvs(self.muJ, self.sigJ, ind).sum() for ind in P]
        )  # Jumps vector
        S_T = S0 * np.exp(
...
class VG_process:
    ...
    def exp_RV(self, S0, T, N):
        ...
        VG = (
            self.theta * G + self.sigma * np.sqrt(G) * Norm
        )  # VG process at final time G
    ...
    def path(self, T=1, N=10000, paths=1):
        ...
        X0 = np.zeros((paths, 1))
        G = ss.gamma(dt / self.kappa, scale=self.kappa).rvs(
            size=(paths, N - 1)
        )  # The gamma RV
...
class GARCH:
    ...
    def fit_from_data(self, data, disp=True):
        ...
        if disp is True:
            ...
            df.loc["omega", ["Params", "SE", "95% CI lower", "95% CI upper"]] *= 10 ** (
                2 * n
            )
            print(df)
    ...
    def log_likelihood(self, R, last_var=True):
        ...
        for i in range(1, N):
            var = (
                self.omega + self.alpha * R[i - 1] ** 2 + self.beta * var
            )  # variance update
...
class OU_process:
    ...
    def path(self, X0=0, T=1, N=10000, paths=1):
        ...
        for t in range(0, N - 1):
            X[:, t + 1] = (
                self.theta
                + np.exp(-self.kappa * dt) * (X[:, t] - self.theta)
                + std_dt * W[:, t]
            )
